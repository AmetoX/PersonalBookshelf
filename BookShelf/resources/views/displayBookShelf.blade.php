...
<body class="font-body min-h-screen">
    ...
    <div class="flex justify-center pb-5">
        <h1 class="text-3xl font-bold">BookShelf</h1>
    </div>

    <!-- Create bar -->
    <div class="flex justify-center">
        <div class="w-1/4">
            <form method="POST" action="{{ route('storeCategory') }}" class="mb-4 relative">
                @csrf
                <input type="text" name="name" placeholder="Enter booklist name" class="w-full px-4 py-2 border border-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-teal-200 focus:border-teal-200 pr-20">
                <button type="submit" class="absolute right-0 top-0 px-5 py-2 text-white bg-teal-600 rounded-md over:bg-teal-700 focus:outline-none focus:ring-teal-500" style="top: 50%; transform: translateY(-50%);">Create Booklist</button>
            </form>
        </div>
    </div>
    <!-- end of create bar -->
    ...
    <div class="w-1/2 mx-auto">
        <h2 class="mb-2 text-2xl font-bold text-gray-800">Your Booklist's:</h2>
        <br>
        @forelse ($categories as $category)
        <div class="border rounded-md p-3 mb-2">
            <div class="flex justify-between items-center">
                <a href="{{ route('displayCategory', ['catId' => $category->id, 'userId' => Auth::user()->id]) }}" class="text-black hover:underline">
                    {{ $category->name }}
                </a>
                <form method="POST" action="{{ route('destroyCategory', ['id' => $category->id, 'name' => $category->name]) }}">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="px-2 py-1 text-white bg-red-500 rounded-md hover:bg-red-600 focus:outline-none focus:bg-red-600">Remove</button>
                </form>
            </div>
        </div>
    ...
    <footer class="fixed inset-x-0 bottom-0 text-center py-3">
        <p>&copy; 2024 Personal BookShelf</p>
    </footer>
